...
export default function AiCustomerDetailsPage() {
    ...
    if (!user || isError) {
        return (
            <div className="rounded-2xl border border-slate-200 bg-white p-8 text-center max-w-md mx-auto mt-12 shadow-xs">
                ...
                <h3 className="text-lg font-semibold text-slate-900">AI Profile Not Found</h3>
                <p className="mt-1 text-sm text-slate-500 mb-6">
                    The requested virtual AI customer profile could not be retrieved or does not exist.
                </p>
                <Link
                    href="/ai-customers"
                ...
                >
                    <ArrowLeft className="size-4" /> Back to AI Customers List
                </Link>
            </div>
        );
    }
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs">
                <div className="border-b border-slate-100 bg-slate-900 p-6 text-white">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-4">
                            {/* Avatar placeholder */}
                            <div className="grid size-16 place-items-center rounded-full bg-indigo-600 text-white font-extrabold text-2xl border-2 border-white/20 shadow-inner shrink-0">
                                <Bot className="size-8 stroke-[2.5]" />
                            </div>
                            ...
                            <div>
                                <div className="flex items-center gap-3 flex-wrap">
                                    ...
                                    <span className="rounded-md bg-indigo-500/30 px-2.5 py-1 text-xs font-extrabold text-indigo-300 border border-indigo-400/30 tracking-wide">
                                        VIRTUAL AI CUSTOMER
                                    </span>
                                </div>
                                <p className="text-sm text-slate-300 mt-1.5 flex items-center gap-3 font-medium">
                                    ...
                                    <span>•</span>
                                    <span>Simulated Email: {user.email}</span>
                                </p>
                            </div>
                        </div>
                ...
                <div className="flex flex-wrap items-center gap-x-8 gap-y-2 p-4 bg-slate-50/80 border-t border-slate-100 text-sm text-slate-700">
                    <div className="flex items-center gap-2">
                        ...
                        <span>Provisioned: <strong className="text-slate-950 font-bold">{createdDate}</strong></span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Clock className="size-4 text-slate-500" />
                        <span>Last Synchronized: <strong className="text-slate-950 font-bold">{updatedDate}</strong></span>
                    </div>
                </div>
            </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between">
                    <div>
                        ...
                        <div className="flex flex-col gap-4.5">
                            <div>
                                <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">AI Persona Name</span>
                                <p className="font-extrabold text-slate-950 text-base">{user.full_name || "N/A"}</p>
                            </div>

                            <div>
                                <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Assigned Email</span>
                            ...
                            <div>
                                <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Simulated Phone</span>
                                <div className="flex items-center gap-2 text-slate-950 font-bold text-sm">
                    ...
                    <div className="pt-5 mt-5 border-t border-slate-100">
                        <span className="text-slate-500 block text-xs mb-2.5 uppercase font-bold tracking-wider">Verification Overrides</span>
                    ...
                    <div>
                        ...
                        {user.profile ? (
                            <div className="flex flex-col gap-4">
                                <div>
                                    <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Simulated Enterprise</span>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div>
                                        <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Entity Model</span>
                                        <p className="font-bold text-slate-950 text-sm">{user.profile.business_type || "N/A"}</p>
                                    </div>
                                    <div>
                                        <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Sector</span>
                                        <p className="font-bold text-slate-950 text-sm">{user.profile.industry_category || user.profile.industry || "N/A"}</p>
                                    </div>
                                </div>

                                <div>
                                    <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Virtual Proprietor</span>
                                ...
                                {user.profile.phone_number && (
                                    <div>
                                        <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Desk Line</span>
                                        <p className="font-bold text-slate-950 text-sm">{user.profile.phone_number}</p>
                                    </div>
                                )}

                                <div>
                                    <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Simulated Hub</span>
                            ...
                            <div className="flex flex-col items-center justify-center py-10 text-center">
                                ...
                                <p className="text-sm font-bold text-slate-800">No Blueprint Configured</p>
                                <p className="text-xs text-slate-500 mt-1 max-w-[200px] leading-relaxed">
                                    This virtual actor does not hold custom business entity parameters.
                                </p>
                            </div>
                        )}
                    </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs flex flex-col justify-between">
                    <div>
                        ...
                        {user.shiping_address ? (
                            <div className="flex flex-col gap-4">
                                <div>
                                    <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Receiver Entity</span>
                                ...
                                {user.shiping_address.email && (
                                    <div>
                                        <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Dispatch Notification</span>
                                ...
                                {user.shiping_address.phone && (
                                    <div>
                                        <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Fulfillment Contact</span>
                                        <p className="font-bold text-slate-950 text-sm">{user.shiping_address.phone}</p>
                                    </div>
                                )}

                                <div>
                                    <span className="text-slate-500 block text-xs mb-1 uppercase font-bold tracking-wider">Simulated Destination</span>
                            ...
                            <div className="flex flex-col items-center justify-center py-10 text-center">
                                ...
                                <p className="text-sm font-bold text-slate-800">No Destination Bound</p>
                                <p className="text-xs text-slate-500 mt-1 max-w-[200px] leading-relaxed">
                                    No targeted physical shipping fulfillment record mapped for this AI bot.
                                </p>
                            </div>
                        )}
                    </div>
