...
import { Pencil, Trash2, Plus, Search, RotateCcw, X, HelpCircle, Loader2, ChevronDown } from "lucide-react";
import { toast } from "sonner";
import { useGetFaqsQuery, useAddFaqMutation, useUpdateFaqMutation, useDeleteFaqMutation } from "@/redux/features/faq/faq.api";
...
const getCategoryColor = (category: string) => {
    if (!category) return COLOR_PALETTES[0];
    let hash = 0;
...
export default function FaqPage() {
    ...
    const dynamicCategories = catResponse?.data?.map((c) => c.title) || ["General"];
    const uniqueCategories = Array.from(new Set(dynamicCategories));

    const filterCategoryOptions = [
        { value: "all", label: "All Categories" },
        ...uniqueCategories.map((title) => ({ value: title, label: title })),
    ];
                                    ...
                                    <div className="flex items-start gap-3 flex-1 min-w-0 pr-4">
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2 mb-2">
                                                <span
                                                    className={`inline-flex items-center text-[11px] font-bold px-2.5 py-0.5 rounded-md border tracking-wide uppercase ${getCategoryColor(faq.category)}`}
                                                >
                                                    {faq.category}
                                                </span>
                                            </div>
